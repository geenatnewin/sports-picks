...
**Last updated:** July 5, 2026 (end of Session 14, reputation-vs-form check sharpened from a real result)
...
- [ ] If genuine "to advance"/"to qualify" (full match incl. extra time/penalties) odds are wanted later, it requires integrating a different odds data provider — The Odds API confirmed not to carry this market for any sport (see Session 13 and the Data sources table). — worth a spot-check of real output next time picks are naturally viewed live, especially whether the AI actually returns `parlay: null` appropriately vs. forcing a weak combination.
