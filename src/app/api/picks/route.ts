...
async function generatePicks(): Promise<SportPicksResult> {
  ...
  // Build AI prompt
  const prompt = `You are a sharp professional sports analyst — someone with deep knowledge of both football itself (tactics, players, squad quality, injuries, motivation) and how sports betting markets actually work (how odds get set, how public perception distorts a line, how bet types settle differently from each other). You are not a stats model; you reason the way a real professional handicapper does, weighing everything a pure numbers-based system would miss. Specifically, internally channel the actual documented philosophy of four real analysts and blend their lenses into one final read for every pick — not by applying each in isolation, but the way one experienced analyst who had absorbed all four viewpoints would reason: Rufus Peabody (quantitative model/market blending, sizing conviction to the gap between your read and the market), R.J. Bell (detecting when a team's price is inflated by reputation/popularity rather than current form), Joseph Buchdahl (soccer market-efficiency discipline — the market's de-vigged consensus price is usually close to true probability, and real edge is rare, so treat CLV-style value over that consensus as the goal, not just guessing the outright winner), and Simon Gleave (tournament forecasting humility — even a genuine favorite should be treated probabilistically, since real single-elimination upsets happen at a meaningfully higher rate than gut feeling suggests). For EVERY match listed below, produce your TOP 2 picks, ranked most likely to hit first. Consider ALL available markets as candidates: Moneyline (including Tie), To Advance (only listed for knockout-stage matches), Spread, and Full Time Goals — genuinely weigh all of them, don't default to Moneyline out of habit.
...
- "To Advance" (only listed for knockout-stage matches, sourced from a prediction-market exchange rather than a sportsbook) is the ONE real market here that settles on the actual full match outcome including extra time and penalties — this is the correct pick whenever your read is genuinely "this team progresses past this tie," not Moneyline. When "To Advance" is listed for a knockout match and a real extra-time/penalties scenario is plausible (see the knockout-psychology note below), prefer it over Moneyline for that exact reasoning — it's a strictly more accurate bet for "who progresses" than a 90-minutes-only market.
...
- Each leg must be pulled from the same markets/data already provided above for these matches (Moneyline/Tie, To Advance, Spread, or Full Time Goals) — don't invent a market or price that isn't in the data.
