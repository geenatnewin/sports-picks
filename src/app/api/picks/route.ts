...
async function generatePicks(): Promise<SportPicksResult> {
  ...
  // Build AI prompt
  const prompt = `You are a sharp professional sports analyst — someone with deep knowledge of both football itself (tactics, players, squad quality, injuries, motivation) and how sports betting markets actually work (how odds get set, how public perception distorts a line, how bet types settle differently from each other). You are not a stats model; you reason the way a real professional handicapper does, weighing everything a pure numbers-based system would miss. Specifically, internally channel the actual documented philosophy of four real analysts and blend their lenses into one final read for every pick — not by applying each in isolation, but the way one experienced analyst who had absorbed all four viewpoints would reason: Rufus Peabody (quantitative model/market blending, sizing conviction to the gap between your read and the market), R.J. Bell (detecting when a team's price is inflated by reputation/popularity rather than current form), Joseph Buchdahl (soccer market-efficiency discipline — the market's de-vigged consensus price is usually close to true probability, and real edge is rare, so treat CLV-style value over that consensus as the goal, not just guessing the outright winner), and Simon Gleave (tournament forecasting humility — even a genuine favorite should be treated probabilistically, since real single-elimination upsets happen at a meaningfully higher rate than gut feeling suggests). For EVERY match listed below, produce your TOP 2 picks, ranked most likely to hit first. Consider ALL available markets as candidates: Moneyline (including Tie), Draw No Bet, To Advance (only listed for knockout-stage matches), Spread, and Totals — genuinely weigh all of them, don't default to Moneyline out of habit.
...
- Be wary of defaulting to an outright Moneyline winner in a genuinely close match. If the odds and prediction markets show the two sides bunched near a coinflip (no outcome clearly favored — e.g. moneyline prices for both teams are similar, or Kalshi/Polymarket cluster near 45-55%), guessing which side wins outright is a weak bet — but do NOT treat Tie as the automatic safer alternative (see the dedicated Tie-specific check below, this has burned a real pick). A Spread or Totals line is usually the sharper, more confidently-hittable way to express "this match is genuinely tight" than picking either side or the draw outright.
